...
Agent.prototype.update_master_address = function(server, address) {
  ...
  for (var s = 0; s < this.doc.small.servers.length; s++) {
    // we need to get the server name from the config value
    var confServerSplit = this.doc.small.servers[s].split(' ');
    var confServer = confServerSplit[confServerSplit.length-1];

    if(confServer == server) {
      // We've found the matching server
      this.doc.small.servers[s] = address + ":1 " + server;
      return;
    };
  };

  for (var s = 0; s < this.doc.session.servers.length; s++) {
    // we need to get the server name from the config value
    var confServerSplit = this.doc.session.servers[s].split(' ');
    var confServer = confServerSplit[confServerSplit.length-1];

    if(confServer == server) {
      // We've found the matching server
      this.doc.session.servers[s] = address + ":1 " + server;
      return;
    };
  };
